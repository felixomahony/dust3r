...
def _interleave_imgs(*imgs):
    assert len(imgs) > 1
    res = {}
    for key, value1 in imgs[0].items():
        values = tuple(img[key] for img in imgs)
        if isinstance(value1, torch.Tensor):
            value = torch.stack(values, dim=1).flatten(0, 1)
        else:
...
def loss_of_one_batch(
    ...
    if symmetrize_batch:
        views = make_batch_symmetric(batch)
...
def check_if_same_size(pairs):
    ...
    return all(
        all(
            pairs[j][i]["img"].shape[-2:] == pairs[0][i]["img"].shape[-2:]
            for i in range(len(pairs[j]))
        )
